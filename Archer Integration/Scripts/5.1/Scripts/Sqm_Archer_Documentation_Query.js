try{
    ...
    var fiscalYearApply = "FY" + fiscalYearFilter.toString().slice(-2);   
...
},
 {
    ...
    $lookup:{
        ...
        $addFields: {
            ...
            'requirementcontrol.controlsByPeriod': { $cond: { if: {$eq: ['$assignment.prevId', '']},then: "Control added in " + fiscalYearApply, else: "Rolled forward Control"} }, //Additional Column controlsByPeriod
    ...
    },
    {
        ...
        IEC: { $concat: ['', {
            $reduce: {
                ...
                initialValue: '',
                in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, { $cond: { if: { $eq: ['$requirementcontrol.controlType._id','ControlType_ItDependentManual'] }, then: {
    ...
    $reduce: {
        ...
        in: {
            ...
            IECSystems : { $cond: { if: { $eq: ['$requirementcontrol.controlType._id','ControlType_ItDependentManual'] }, then: {
                $reduce: {
                    input: {
                        $map: {
                            input: "$requirementcontrol.addInformationExecutionControls",
           ...
           /* IsThisIECReportOrOther: {
                $reduce: {
                    input: '$requirementcontrol.iecReportsName',
                    initialValue: '',
                    in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, '$$this.name'] }
    ...
    $reduce: {
        ...
        in: {
            ...
            /*ControlOwnerProceduresOverCompleteness: {
                $reduce: {
                    input: '$requirementcontrol.addInformationExecutionControls.iecOwnerProcedures',
                    initialValue: '',
                    in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, '$$this'] }
            ...
            ControlOwnerProceduresOverCompletenessLocal: {
                $reduce: {
                    input: {$filter: {input: '$requirementcontrol.addInformationExecutionControls.iecOwnerProceduresAlt',cond: { $ne: ['$$this', ''] }}},
                    initialValue: '',
                    in: {$concat: ['$$value',{ $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } },'$$this']}
                 }
             },*/
            ControlOwnerProceduresOverCompleteness: {
                $reduce: {
                    ...
                    initialValue: '',
                    in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, { $cond: { if: { $eq: ['$requirementcontrol.controlType._id','ControlType_ItDependentManual'] }, then: {
    $concat: ['IEC-ID-',"$$this.uniqueId",':']}, else: '' } },'$$this.iecOwnerProcedures'] }
                }
            },
            ControlOwnerProceduresOverCompletenessLocal: {
                $reduce: {
...
},
{
    ...
    $lookup: {
        ...
        $lookup: {
            from: 'keycontrolresource',
            let:{
                reqCtrlAddIecReports:'$keyControl.addInformationExecutionControls.iecReports'
            },
            pipeline: [{
                        $match: {
                            $expr: {
...
},
{
       ...
       'ProcessOwnerTitlesId': {
        ...
        IEC: { $concat: ['', {
            $reduce: {
                ...
                initialValue: '',
                in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, { $cond: { if: { $eq: ['$keyControl.controlType._id','ControlType_ItDependentManual'] }, then: {
    $concat: ['IEC-ID-',"$$this.uniqueId",':']
}, else: '' } },'$$this.name'] }
    ...
    $reduce: {
        ...
        in: {
            ...
            IECSystems: { $cond: { if: { $eq: ['$keyControl.controlType._id','ControlType_ItDependentManual'] }, then: {
                $reduce: {
                    input: {
                        $map: {
                            input: "$keyControl.addInformationExecutionControls",
            ...
            /*IsThisIECReportOrOther: {
                $reduce: {
                    input: '$keyControl.iecReportsName',
                    initialValue: '',
                    in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, '$$this.name'] }
    ...
    $reduce: {
        ...
        in: {
            ...
            /*ControlOwnerProceduresOverCompleteness: {
                $reduce: {
                    input: '$keyControl.addInformationExecutionControls.iecOwnerProcedures',
                    initialValue: '',
                    in: { $concat: ['$$value', { $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } }, '$$this'] }
            ...
            ControlOwnerProceduresOverCompletenessLocal: {
                $reduce: {
                    input: {$filter: {input: '$keyControl.addInformationExecutionControls.iecOwnerProceduresAlt',cond: { $ne: ['$$this', ''] }}},
                    initialValue: '',
                    in: {$concat: ['$$value',{ $cond: { if: { $eq: ['$$value', ''] }, then: '', else: '; ' } },'$$this']}
                 }
             },*/
    ...
    $concat: ['IEC-ID-',"$$this.uniqueId",':']}, else: '' } },'$$this.iecOwnerProcedures'] }
            ...
            ControlsByPeriod: {
                $cond: {
                    ...
                    then: "Control added in " + fiscalYearApply,
...
db.documentation.aggregate([{
    ...
    $project: {
        ...
        PCAOB_Configured_Object: {
             $cond: { if: { $eq: ['$isPCAOBRegistered', true] }, then: 'Yes', else: 'No' }
         },//Added New column PCAOB_Configured_Object QSR
    }
},
    {
    $addFields: { "FiscalYear": fiscalYearApply }
...
db.documentation.aggregate([{
    ...
    $project: {
        ...
        PCAOB_Configured_Object: {
             $cond: { if: { $eq: ['$isPCAOBRegistered', true] }, then: 'Yes', else: 'No' }
         },//Added New column PCAOB_Configured_Object QO
    }
},
    {
    $addFields: { "FiscalYear": fiscalYearApply }
...
db.sqmarcherqualityobjective.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc){
    ...
    var updatedEvent = existingQO && db.event.find({publisher: kc.EntityId,actor: kc.QualityObjectiveUniqueID, fiscalYear: fiscalYearFilter, message: 'ActionType_Update',modifiedOn : {$gt:existingQO.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isQOUpdate = existingQO && updatedEvent.length>0;
        if (!(existingQO)) {
            var getNewEventDateQO = db.event.find({publisher: kc.EntityId,actor: kc.QualityObjectiveUniqueID, fiscalYear: fiscalYearFilter, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
...
db.sqmarcherqualityobjectivemaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(kc){
    ...
    var isQODelete = db.event.find({publisher: kc.EntityId,actor: kc.QualityObjectiveUniqueID, fiscalYear: fiscalYearFilter, message: 'ActionType_Delete',modifiedOn : {$gt:kc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
...
db.sqmarcherqualityrisk.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc){
    ...
    var updatedEventQR = existingQR && db.event.find({publisher: kc.EntityId,actor: kc.QualityRiskUniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Update',modifiedOn : {$gt:existingQR.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isQRUpdate = existingQR && updatedEventQR.length>0;
        if (!(existingQR)) {
            var getNewEventDateQR = db.event.find({publisher: kc.EntityId, fiscalYear: fiscalYearFilter,actor: kc.QualityRiskUniqueId, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
...
db.sqmarcherqualityriskmaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(kc){
    ...
    var isQRDelete = db.event.find({publisher: kc.EntityId,actor: kc.QualityRiskUniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Delete',modifiedOn : {$gt:kc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var frimInfo = db.firm.findOne({abbreviation: kc.EntityId, fiscalYear:fiscalYearFilter});
...
db.sqmarchersubrisk.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc){
    ...
    var updatedEventSR = existingSR && db.event.find({publisher: kc.EntityId,actor: kc.QualitySubRiskUniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Update',modifiedOn : {$gt:existingSR.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isSRUpdate = existingSR && updatedEventSR.length>0;
        if (!(existingSR)) {
            var getNewEventDateSR = db.event.find({publisher: kc.EntityId,actor: kc.QualitySubRiskUniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
...
db.sqmarchersubriskmaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(kc){
    ...
    var isSRDelete = db.event.find({publisher: kc.EntityId,actor: kc.QualitySubRiskUniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Delete',modifiedOn : {$gt:kc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var frimInfo = db.firm.findOne({abbreviation: kc.EntityId, fiscalYear:fiscalYearFilter});
...
db.sqmarchersubrisk.aggregate( [   
    ...
    db.sqmarcherkeycontrol.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc) {
        ...
        var functionLeaderTitlesInfo = db.sqmarcherfunction.find({ FunctionId: {$in:functionIds}, EntityId:kc.EntityId }, { FunctionLeaderTitle:1,FunctionLeaderTitleId:1}).toArray();
    ...
    var eventUpdate = existingControl && db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Update',modifiedOn : {$gt:existingControl.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isControlUpdate = existingControl  && eventUpdate.length>0;
    
    var eventUnreject = existingControl && db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_ManualUnreject',modifiedOn : {$gt:existingControl.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isControlUnReject = existingControl && eventUnreject.length>0;
        if (!(existingControl)) {
            var getNewEventDate = db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
       ...
       else
        {
            if(existingControl.LastPublishedRecordStatus === 'Inactive')
            {
              ...
              var event = db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: {$in:['ActionType_ManualUnreject','ActionType_Update','ActionType_Add']}}).sort({'modifiedOn':-1}).limit(1).count()>0 ? db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: {$in:['ActionType_ManualUnreject','ActionType_Update','ActionType_Add']}}).sort({'modifiedOn':-1}).limit(1).toArray():[];
            ...
            else
            {
               ...
               var isUpdateFromOutside = [];
                ...
                isUpdateFromOutside = db.event.find({publisher: kc.EntityId, fiscalYear: fiscalYearFilter,"actor": { $in: function_process_ids },actorType: {$in:['Function','Process']}
                        , message: {$in:['ActionType_Function_Title_Add','ActionType_Function_Title_Update','ActionType_Function_Title_Remove','ActionType_Function_NotApplicable'
               ...
               // Find Updates in Control Owner/Operator Titles
                ...
                var isTitleUpdateded = db.event.find({publisher: kc.EntityId, fiscalYear: fiscalYearFilter,"actor": { $in: all_TitleIds_In_ObjectIds },actorType: {$in:['GlobalTitle','CustomTitle']}
               ...
               {
                var rejectedQRs = db.action.find({fiscalYear:fiscalYearFilter,objectId:{$in:kc.qualityRiskUniqueIdArray}, firmId:kc.EntityId},{objectId:1}).toArray();
               ...
               {
                   var isObjectUpdated = db.event.find({publisher: 'global',"actor": { $in: function_process_ids },actorType: {$in:['Function','Process']}
                        , message: {$in:['ActionType_Update']}, fiscalYear: fiscalYearFilter
                        ,modifiedOn : {$gt:existingControl.ArcherPublishedOn}})
                        .sort({'modifiedOn':-1}).limit(1).toArray();
...
db.sqmarcherkeycontrolmaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(control){
        ...
        if (!(existingControl)) {
            var eventDelete = db.event.find({publisher: control.EntityId,actor: control.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_Delete',modifiedOn : {$gt:control.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
			var isControlDelete = eventDelete.length>0;
            var eventRejected = db.event.find({publisher: control.EntityId,actor: control.Ref_UniqueId, fiscalYear: fiscalYearFilter, message: 'ActionType_ManualReject',modifiedOn : {$gt:control.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
            ...
            else if(isControlRejected)
            {
                ...
                if(control.EventType === 'Rejected'){
                    ...
                    if(getAssignment.length)
                    {
                     ...
                     {
                         var deletedByParent = db.event.find({actor: control.Ref_UniqueId, message: 'ActionType_Delete', fiscalYear: fiscalYearFilter,modifiedOn : {$gt:control.ArcherPublishedOn}, "$or": [{publisher : {$in:parents}},{publisher:control.EntityId}]}).sort({'modifiedOn':-1}).toArray();
...
db.sqmarcherresources.aggregate([{
 ...
 db.sqmarcherresources.find({EntityId: { $in: publishedEntityIds }}).forEach(function (res){
    ...
    var updatedEventRes = existingRes && db.event.find({publisher: res.EntityId,actor: ObjectId(res.UniqueId), fiscalYear: fiscalYearFilter, message: 'ActionType_Update',modifiedOn : {$gt:existingRes.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isResUpdate = existingRes && updatedEventRes.length>0;
        if (!(existingRes)) {
            var getNewEventDateRes = db.event.find({publisher: res.EntityId,actor: ObjectId(res.UniqueId), fiscalYear: fiscalYearFilter, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
...
db.sqmarcherresourcesmaster.find({EntityId: { $in: publishedEntityIds }}).forEach(function(res){
    ...
    var isResDelete = db.event.find({publisher: res.EntityId,actor: ObjectId(res.UniqueId), fiscalYear: fiscalYearFilter, message: 'ActionType_Delete',modifiedOn : {$gt:res.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
...
//Functions Event Log
db.sqmarcherfunction.find({EntityId: { $in: publishedEntityIds }}).forEach(function (func){
    var existingFunc = db.sqmarcherfunctionmaster.findOne({UniqueRowId: func.UniqueRowId});
    
    //UPDATE
    var updatedFunction = existingFunc && db.event.find({publisher:'global',actor: ObjectId(func.FunctionId), fiscalYear: fiscalYearFilter, message: 'ActionType_Update'
        ,modifiedOn : {$gt:existingFunc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    ...
    if(!(existingFunc)){
       ...
       var getNewEventDate = db.event.find({publisher: {$in:[func.EntityId,'global']},actor: {$in:ids}, fiscalYear: fiscalYearFilter, message: {$in:['ActionType_Function_Title_Add'
           ,'ActionType_Title_Users_Add','ActionType_Add','ActionType_Function_Entity_Assigned']}}).sort({'modifiedOn':-1}).limit(1).toArray();
        db.sqmarcherfunction.updateOne({_id:func._id},{$set:{EventType:'New',EventAction : 'Update', LastPublishedRecordStatus : 'Active'
        ...
        db.sqmarcherfunction.updateOne({_id:func._id},{$set:{EventType:'Updated',EventAction : 'Update', LastPublishedRecordStatus : 'Active',LastProcessedDate:new Date().toISOString()
        ...
        var ids = [ObjectId(func.FunctionId)];
            ...
            isUpdateFromOutside = db.event.find({publisher:{$in:[func.EntityId,'global']} , fiscalYear: fiscalYearFilter,"actor":{$in:ids},actorType: {$in:['Function','GlobalTitle','CustomTitle']}
                    , message: {$in:['ActionType_Function_Title_Add','ActionType_Title_Users_Add','ActionType_Function_Title_Update','ActionType_Function_Title_Remove','ActionType_Title_Users_Update'
                        ,'ActionType_Title_Users_Remove','ActionType_Update']},modifiedOn : {$gt:existingFunc.ArcherPublishedOn}})
                    .sort({'modifiedOn':-1}).limit(1).toArray();
           ...
           db.sqmarcherfunction.updateOne({_id:func._id},{$set:{EventType:existingFunc.EventType,EventAction : existingFunc.EventAction, LastPublishedRecordStatus : existingFunc.LastPublishedRecordStatus,LastProcessedDate:new Date().toISOString()
               ,EventDate : existingFunc.EventDate}});
        }
})

//DELETE FUNCTION
db.sqmarcherfunctionmaster.find({EntityId: { $in: publishedEntityIds }}).forEach(function(func){
    var existingFunction = db.sqmarcherfunction.findOne({UniqueRowId: func.UniqueRowId});
   ...
   // TO FIND UPDATES IN FUNCTIONS TITLEs
    ...
    var funcDeleteData = db.event.find({publisher:{$in:[func.EntityId,'global']},actor: {$in:ids}, fiscalYear: fiscalYearFilter,message: {$in:['ActionType_Function_Title_Update','ActionType_Function_Title_Remove'
        ,'ActionType_Title_Users_Update','ActionType_Title_Users_Remove','ActionType_Delete']}}).sort({'modifiedOn':-1}).limit(1).toArray();

    var funcNAData = db.event.find({publisher:func.EntityId,actor: ObjectId(func.FunctionId), fiscalYear: fiscalYearFilter,message:'ActionType_Function_NotApplicable'}).sort({'modifiedOn':-1}).limit(1).toArray();
...
//Update isPublishQueryRun property
 ...
 db.firm.aggregate([
            {
                $match: {
                    abbreviation: { $in: publishedEntityIds },
                    fiscalYear: { $eq: fiscalYearFilter},
                    $expr: {
                        $and: [
                               { $eq: ['$isPublishQueryRun', false] },
                               {
