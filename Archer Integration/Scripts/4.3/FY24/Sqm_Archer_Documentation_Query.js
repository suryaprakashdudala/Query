...
}]).toArray().map(f => f.abbreviation); // ['GE','IN']
		...
		pipeline: [{
                    $match: {
                        $expr: {
							$and: [
								{ $eq: ["$fiscalYear", fiscalYearFilter]},
								{ $in:['$abbreviation',publishedEntityIds]}
							]
                            /*$and: [
                                '$publishedDate',
                                { $ne: ['$publishedDate', ''] },
...
db.documentation.aggregate([{
    ...
    $match: {
        $expr: {
			$and: [
				{ $eq: ["$firm.fiscalYear", fiscalYearFilter]},
				{ $in:['$firm.abbreviation',publishedEntityIds]}
			]
            /*$and: [
                '$firm.publishedDate',
...
db.sqmarcherqualityobjective.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc){
    var existingQO = db.sqmarcherqualityobjectivemaster.findOne({EntityId: kc.EntityId,QualityObjectiveUniqueID:kc.QualityObjectiveUniqueID});
    var updatedEvent = db.event.find({publisher: kc.EntityId,actor: kc.QualityObjectiveUniqueID, message: 'ActionType_Update',modifiedOn : {$gt:existingQR.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isQOUpdate = existingQO && updatedEvent.length>0;
        if (!(existingQO)) {
            var getNewEventDateQO = db.event.find({publisher: kc.EntityId,actor: kc.QualityObjectiveUniqueID, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
            db.sqmarcherqualityobjective.updateOne({_id:kc._id},{$set:{EventType:'New',EventAction : 'Update', RecordStatus : 'Active',EventDate : getNewEventDateQO.length>0?getNewEventDateQO[0].modifiedOn:''}})
        }
        else if (isQOUpdate) {
            db.sqmarcherqualityobjective.updateOne({_id:kc._id},{$set:{EventType:'Updated',EventAction : 'Update', RecordStatus : 'Active',EventDate : updatedEvent[0].modifiedOn}})
        }
       else
        {
            db.sqmarcherqualityobjective.updateOne({_id:kc._id},{$set:{EventType:existingQO.EventType,EventAction : existingQO.EventAction, RecordStatus : existingQO.RecordStatus, EventDate : existingQO.EventDate}})
        }
})
db.sqmarcherqualityobjectivemaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(kc){
    var existingQO = db.sqmarcherqualityobjective.findOne({EntityId: kc.EntityId,QualityObjectiveUniqueID :kc.QualityObjectiveUniqueID});
    var isQODelete = db.event.find({publisher: kc.EntityId,actor: kc.QualityObjectiveUniqueID, message: 'ActionType_Delete',modifiedOn : {$gt:kc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var frimInfo = db.firm.findOne({abbreviation: kc.EntityId, fiscalYear:fiscalYearFilter});
    if(!existingQO && isQODelete.length>0){
            kc.EventType = 'Deleted';
            kc.EventAction = 'Update';
            kc.RecordStatus= 'Inactive';
            kc.PublishedDate = frimInfo.publishedDate;
            kc.EventDate = isQODelete[0].modifiedOn;
            db.sqmarcherqualityobjective.insertOne(kc);
            }
    else 
            {
				db.sqmarcherqualityobjective.updateOne({_id:kc._id},{$set:{EventType:existingQO.EventType,EventAction : existingQO.EventAction, RecordStatus : existingQO.RecordStatus, PublishedDate: frimInfo.publishedDate, EventDate : existingQO.EventDate}});
            }
})
...
db.sqmarcherqualityrisk.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc){
    ...
    var updatedEventQR = db.event.find({publisher: kc.EntityId,actor: kc.QualityRiskUniqueId, message: 'ActionType_Update',modifiedOn : {$gt:existingQR.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isQRUpdate = existingQR && updatedEventQR.length>0;
        if (!(existingQR)) {
            var getNewEventDateQR = db.event.find({publisher: kc.EntityId,actor: kc.QualityRiskUniqueId, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
            db.sqmarcherqualityrisk.updateOne({_id:kc._id},{$set:{EventType:'New',EventAction : 'Update', RecordStatus : 'Active', EventDate : getNewEventDateQR.length>0?getNewEventDateQR[0].modifiedOn:''}});
                }
        else if (isQRUpdate) {
            db.sqmarcherqualityrisk.updateOne({_id:kc._id},{$set:{EventType:'Updated',EventAction : 'Update', RecordStatus : 'Active', EventDate : updatedEventQR[0].modifiedOn}})
                        }
       else
        {
            db.sqmarcherqualityrisk.updateOne({_id:kc._id},{$set:{EventType:existingQR.EventType,EventAction : existingQR.EventAction, RecordStatus : existingQR.RecordStatus, EventDate : existingQR.EventDate}})
                            }
})
db.sqmarcherqualityriskmaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(kc){
    ...
    var isQRDelete = db.event.find({publisher: kc.EntityId,actor: kc.QualityRiskUniqueId, message: 'ActionType_Delete',modifiedOn : {$gt:kc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var frimInfo = db.firm.findOne({abbreviation: kc.EntityId, fiscalYear:fiscalYearFilter});
    if(!existingQR && isQRDelete.length>0){
            ...
            db.sqmarcherqualityrisk.insertOne(kc);
                    ...
                    {
            db.sqmarcherqualityrisk.updateOne({_id:kc._id},{$set:{EventType:existingQR.EventType,EventAction : existingQR.EventAction, RecordStatus : existingQR.RecordStatus, PublishedDate: frimInfo.publishedDate, EventDate : existingQR.EventDate}});
                     }
})
...
db.sqmarchersubrisk.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc){
    var existingSR = db.sqmarchersubriskmaster.findOne({EntityId: kc.EntityId,QualitySubRiskUniqueId :kc.QualitySubRiskUniqueId});
    var updatedEventSR = db.event.find({publisher: kc.EntityId,actor: kc.QualitySubRiskUniqueId, message: 'ActionType_Update',modifiedOn : {$gt:existingSR.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isSRUpdate = existingSR && updatedEventSR.length>0;
        if (!(existingSR)) {
            var getNewEventDateSR = db.event.find({publisher: kc.EntityId,actor: kc.QualitySubRiskUniqueId, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
            db.sqmarchersubrisk.updateOne({_id:kc._id},{$set:{EventType:'New',EventAction : 'Update', RecordStatus : 'Active',EventDate : getNewEventDateSR.length>0?getNewEventDateSR[0].modifiedOn:''}});
                     }
        else if (isSRUpdate) {
            db.sqmarchersubrisk.updateOne({_id:kc._id},{$set:{EventType:'Updated',EventAction : 'Update', RecordStatus : 'Active', EventDate : updatedEventSR[0].modifiedOn}});
                }
       else
        {
            db.sqmarchersubrisk.updateOne({_id:kc._id},{$set:{EventType:existingSR.EventType,EventAction : existingSR.EventAction, RecordStatus : existingSR.RecordStatus, EventDate : existingSR.EventDate}});
            }
})
db.sqmarchersubriskmaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(kc){
    var existingSR = db.sqmarchersubrisk.findOne({EntityId: kc.EntityId,QualitySubRiskUniqueId :kc.QualitySubRiskUniqueId});
    var isSRDelete = db.event.find({publisher: kc.EntityId,actor: kc.QualitySubRiskUniqueId, message: 'ActionType_Delete',modifiedOn : {$gt:kc.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var frimInfo = db.firm.findOne({abbreviation: kc.EntityId, fiscalYear:fiscalYearFilter});
    if(!existingSR && isSRDelete.length>0){
        ...
        {
            db.sqmarchersubrisk.updateOne({_id:kc._id},{$set:{EventType:existingSR.EventType,EventAction : existingSR.EventAction, RecordStatus : existingSR.RecordStatus, PublishedDate: frimInfo.publishedDate,EventDate : existingSR.EventDate}});
        }
})
...
db.sqmarchersubrisk.aggregate( [   
    ...
    db.sqmarcherkeycontrol.find({EntityId :{$in:publishedEntityIds}}).forEach(function (kc) {
        ...
        var getParents = db.firm.aggregate([{
                        ...
                        var rejectedQRs = db.action.find({fiscalYear:fiscalYearFilter,objectId:{$in:assignmentData}, "$or": [{firmId : {$in:parents}},{firmId:kc.EntityId}]},{objectId:1}).toArray().map(f => f.objectId).filter(Boolean).join(';');;
                        ...
                        kc.qualityRiskUniqueIdArray.forEach(function(rc){
                            ...
                            kc.QualityRiskUniqueIds = qualityRisksAssociated;

                        if(kc.QualitySubRiskUniqueIds !="")
                        {
                            var subRisksAssociated = "";
                            var subRisksRejected = "";
                            kc.QualitySubRiskUniqueIds.split(';').forEach(function(sr)
                            {
                                var getSrsParents = db.documentation.find({fiscalYear:fiscalYearFilter, uniqueId:sr},{relatedQualityRisks:1,_id:0}).toArray().map(f => f.relatedQualityRisks)[0];;
                            ...
                            kc.QualitySubRiskUniqueIds = subRisksAssociated;
                            kc.RejectedSubQualityRiskUniqueIds = subRisksRejected;
                        }
        db.sqmarcherkeycontrol.updateOne({_id:kc._id},{$set:{FunctionName:functionNames,FunctionOwnerTitle : functionLeaderTitle, FunctionOwnerAssignments : functionLeaderTitleAssignment,QualityRiskUniqueIds : kc.QualityRiskUniqueIds, RejectedQualityRiskUniqueIds: kc.RejectedQualityRiskUniqueIds,QualitySubRiskUniqueIds : kc.QualitySubRiskUniqueIds, RejectedSubQualityRiskUniqueIds: kc.RejectedSubQualityRiskUniqueIds}})

    var existingControl = db.sqmarcherkeycontrolmaster.findOne({EntityId: kc.EntityId,Ref_UniqueId :kc.Ref_UniqueId});
    var eventUpdate = existingControl && db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, message: 'ActionType_Update',modifiedOn : {$gt:existingControl.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isControlUpdate = existingControl  && eventUpdate.length>0;
    
    var eventUnreject = existingControl && db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, message: 'ActionType_ManualUnreject',modifiedOn : {$gt:existingControl.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isControlUnReject = existingControl && eventUnreject.length>0;
        if (!(existingControl)) {
            var getNewEventDate = db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
			
            db.sqmarcherkeycontrol.updateOne({_id:kc._id},{$set:{EventType:'New',EventAction : 'Update', RecordStatus : 'Active',EventDate : getNewEventDate.length>0?getNewEventDate[0].modifiedOn:''}});
        }
        else if (isControlUnReject) {
            db.sqmarcherkeycontrol.updateOne({_id:kc._id},{$set:{EventType:'Unrejected',EventAction : 'Update', RecordStatus : 'Active',EventDate : eventUnreject[0].modifiedOn}});
        }
        else if (isControlUpdate) {
			var eventName = existingControl.EventType === 'Rejected'? 'Unrejected' : 'Updated';
            db.sqmarcherkeycontrol.updateOne({_id:kc._id},{$set:{EventType:eventName,EventAction : 'Update', RecordStatus : 'Active',EventDate : eventUpdate[0].modifiedOn}});
        }
       else
        {
            if(existingControl.RecordStatus === 'Inactive')
            {
				var events ={
              ...
              var event = db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, message: {$in:['ActionType_ManualUnreject','ActionType_Update','ActionType_New']}}).sort({'modifiedOn':-1}).limit(1).count()>0 ? db.event.find({publisher: kc.EntityId,actor: kc.Ref_UniqueId, message: {$in:['ActionType_ManualUnreject','ActionType_Update','ActionType_New']}}).sort({'modifiedOn':-1}).limit(1).toArray():[];
              ...
              existingControl.RecordStatus = 'Active';
			  ...
			  db.sqmarcherkeycontrol.updateOne({_id:kc._id},{$set:{EventType: eventName, EventAction : existingControl.EventAction, RecordStatus : existingControl.RecordStatus, EventDate : existingControl.EventDate}});
			}
			else
			{
			    db.sqmarcherkeycontrol.updateOne({_id:kc._id},{$set:{EventType: existingControl.EventType, EventAction : existingControl.EventAction, RecordStatus : existingControl.RecordStatus, EventDate : existingControl.EventDate}});
            }

            if(kc.ControlOperatorTitleAssignments !== existingControl.ControlOperatorTitleAssignments || kc.ControlOwnerTitleAssignments !== existingControl.ControlOwnerTitleAssignments || functionLeaderTitle !== existingControl.FunctionOwnerTitle || functionLeaderTitleAssignment !== existingControl.FunctionOwnerAssignments || kc.ProcessOwnerTitles !== existingControl.ProcessOwnerTitles || kc.ProcessOwnerAssignments !== existingControl.ProcessOwnerAssignments || kc.RejectedQualityRiskUniqueIds !== existingControl.RejectedQualityRiskUniqueIds ||kc.RejectedSubQualityRiskUniqueIds !== existingControl.RejectedSubQualityRiskUniqueIds)
            {
                db.sqmarcherkeycontrol.updateOne({EntityId:kc.EntityId,Ref_UniqueId:kc.Ref_UniqueId},{$set:{EventType:'Updated',EventAction : 'Update', RecordStatus : 'Active', EventDate : /*TODO*/frimInfo.publishedDate}});
            }
        }
});
...
db.sqmarcherkeycontrolmaster.find({EntityId :{$in:publishedEntityIds}}).forEach(function(control){
        ...
        var frimInfo = db.firm.findOne({abbreviation: control.EntityId, fiscalYear:fiscalYearFilter});
        
        if (!(existingControl)) {
            var eventDelete = db.event.find({publisher: control.EntityId,actor: control.Ref_UniqueId, message: 'ActionType_Delete',modifiedOn : {$gt:control.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
			var isControlDelete = eventDelete.length>0;
            var eventRejected = db.event.find({publisher: control.EntityId,actor: control.Ref_UniqueId, message: 'ActionType_ManualReject',modifiedOn : {$gt:control.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
            var isControlRejected = eventRejected.length>0;

            control.PublishedDate = frimInfo.publishedDate;
            ...
            else if(isControlRejected)
            {
                ...
                if(control.EventType === 'Rejected'){
                    ...
                    var getAssignment = db.documentation.find({firmId:control.EntityId,uniqueId:control.Ref_UniqueId, fiscalYear:fiscalYearFilter},{mitigatedResources:1,supportingITApplication:1,relatedQualityRisks:1}).toArray();
                    if(getAssignment.length)
                    {
                      ...
                      if(!isRejected && ((assignmentData.mitigatedResources && assignmentData.mitigatedResources.length) || (assignmentData.supportingITApplication && assignmentData.supportingITApplication.length)))
                      {
                        ...
                        var uniqueResources = mitigatedResourcesArray.concat(supportingITApplicationArray.filter((item) => mitigatedResourcesArray.indexOf(item) < 0));
                         isRejected = db.action.find({fiscalYear:fiscalYearFilter,objectId:{$in:uniqueResources}, "$or": [{firmId : {$in:parents}},{firmId:control.EntityId}]}).count() >0;
                      }
                    }
                     ...
                     {
                         var deletedByParent = db.event.find({actor: control.Ref_UniqueId, message: 'ActionType_Delete',modifiedOn : {$gt:control.ArcherPublishedOn}, "$or": [{publisher : {$in:parents}},{publisher:control.EntityId}]}).sort({'modifiedOn':-1}).toArray();
        ...
        {
            db.sqmarcherkeycontrol.updateOne({EntityId: control.EntityId,UniqueControlID: control.UniqueControlID},{$set:{EventType:existingControl.EventType,EventAction : existingControl.EventAction, RecordStatus : existingControl.RecordStatus, PublishedDate:frimInfo.publishedDate, EventDate : existingControl.EventDate}});
        }
    });
...
db.sqmarcherresources.aggregate([{
 ...
 db.sqmarcherresources.find({EntityId: { $in: publishedEntityIds }}).forEach(function (res){
    ...
    var updatedEventRes = existingRes && db.event.find({publisher: res.EntityId,actor: ObjectId(res.UniqueId), message: 'ActionType_Update',modifiedOn : {$gt:existingRes.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    var isResUpdate = existingRes && updatedEventRes.length>0;
        if (!(existingRes)) {
            var getNewEventDateRes = db.event.find({publisher: res.EntityId,actor: ObjectId(res.UniqueId), message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
           
            db.sqmarcherresources.updateOne({_id:res._id},{$set:{EventType:'New',EventAction : 'Update', RecordStatus : 'Active',EventDate : getNewEventDateRes.length>0?getNewEventDateRes[0].modifiedOn:''}});
        }
        else if (isResUpdate) {
            db.sqmarcherresources.updateOne({_id:res._id},{$set:{EventType:'Updated',EventAction : 'Update', RecordStatus : 'Active', EventDate : updatedEventRes[0].modifiedOn}});
        }
       else
        {
            db.sqmarcherresources.updateOne({_id:res._id},{$set:{EventType:existingRes.EventType,EventAction : existingRes.EventAction, RecordStatus : existingRes.RecordStatus, PublishedDate: existingRes.PublishedDate,EventDate : existingRes.EventDate}});
        }
})
db.sqmarcherresourcesmaster.find({EntityId: { $in: publishedEntityIds }}).forEach(function(res){
    ...
    var isResDelete = db.event.find({publisher: res.EntityId,actor: ObjectId(res.UniqueId), message: 'ActionType_Delete',modifiedOn : {$gt:res.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
    if(!existingRes && isResDelete.length>0){
        var frimInfo = db.firm.findOne({abbreviation: res.EntityId, fiscalYear:fiscalYearFilter});
            res.EventType = 'Deleted';
            res.EventAction = 'Update';
            res.RecordStatus= 'Inactive';
            res.PublishedDate = frimInfo.publishedDate;
...
//Update isPublishQueryRun property
 db.firm.updateMany({ isPublishQueryRun: false, abbreviation: { $in: publishedEntityIds } }, { $set: { isPublishQueryRun: true } }, { multi: true })
 } catch (error) {
    db.sqmarcherkeycontroltemp.drop();
    db.sqmarcherqualityrisktemp.drop();
    db.sqmarcherqualityobjectivetemp.drop();
