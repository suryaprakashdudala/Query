 try{ 
    ...
    db.globaldocumentation.aggregate( [
        ...
        {
        $addFields: {
            FiscalYear: { $concat: ["FY", { $substr: ["$EntityFiscalYear", 2, 3] }] },
    ...
    db.masterDataProcess.find().forEach(function (pro){
        var existingProcess = db.masterDataProcessCopy.findOne({UniqueId: pro.UniqueId, FiscalYear: pro.FiscalYear});
        var isProcessUpdate = existingProcess && db.event.find({actor: ObjectId(pro._id), fiscalYear: pro.EntityFiscalYear, message: 'ActionType_Update',modifiedOn : {$gt:existingProcess.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();

            if (!(existingProcess)) {
                var getNewEventDateProcess = db.event.find({actor: ObjectId(pro._id), fiscalYear: pro.EntityFiscalYear, message: 'ActionType_Add'}).sort({'modifiedOn':-1}).limit(1).toArray();
    ...
    db.masterDataProcessCopy.find().forEach(function(pro){
        var existingProcess = db.masterDataProcess.findOne({UniqueId :pro.UniqueId, FiscalYear: pro.FiscalYear});
        var isProcessDelete = db.event.find({actor: pro.UniqueId, fiscalYear: pro.EntityFiscalYear, message: 'ActionType_Delete',modifiedOn : {$gt:pro.ArcherPublishedOn}}).sort({'modifiedOn':-1}).limit(1).toArray();
